...
public class HologramsBacklightCommand implements Command<CommandSourceStack> {
    ...
    public int executeCommand(CommandContext<CommandSourceStack> context, String[] args) {
        ...
        if (!UtilPermissions.hasPermission(source, UtilPermissions.BACKLIGHT)) {            source.sendSystemMessage(UtilChatColour.parse("&c&l(!) &cYou don't have permission to use the backlight command."));
            return 0;
        }

        if (args.length < 2) {
            source.sendSystemMessage(UtilChatColour.parse("&c&l(!) &cUsage: /eh backlight <id> <on|off|toggle> [height 1-10]"));
            return 0;
        }
        ...
        if (hologram == null) {
            source.sendSystemMessage(UtilChatColour.parse("&c&l(!) &cHologram '&f" + id + "&c' not found."));
            return 0;
        }
        if (!(hologram instanceof ForgeHologram)) {
            source.sendSystemMessage(UtilChatColour.parse("&c&l(!) &cThis hologram type does not support backlights."));
            return 0;
        }
        ...
        if (args.length >= 3) {
            try {
                ...
                source.sendSystemMessage(UtilChatColour.parse("&c&l(!) &cHeight must be a number 1-10."));
                return 0;
            }
        }
        ...
        switch (mode) {
            ...
            case "toggle":
                ...
                break;
            default:
                source.sendSystemMessage(UtilChatColour.parse("&c&l(!) &cUnknown mode: &f" + mode + "&c. Use on/off/toggle."));
                return 0;
        }
        ...
        try {
            ...
            source.sendSystemMessage(UtilChatColour.parse("&c&l(!) &cError applying backlight: &f" + e.getMessage()));
            return 0;
        }
