...
public class HologramTextDisplay extends Display.TextDisplay {
    ...
    public HologramTextDisplay(Level level) {
        super(EntityType.TEXT_DISPLAY, level);
    }

    /**
     * Applies the display settings shared by every hologram line: a fixed billboard so the
     * text keeps the hologram's rotation, centred text, no wrapping, and the configured
     * background.
     *
     * <p>The remaining vanilla defaults already match how a nameplate looks - no shadow, not
     * see-through - so they are left alone.
     */
     ...
     * custom background. Writing the full state keeps the behaviour identical on 1.20.1, where
     * the same read is conditional on the key being present.
     */
    private void applySettings() {
        this.readAdditionalSaveData(buildSettingsTag());
    }
    ...
    public List<SynchedEntityData.DataValue<?>> buildTextData(Component text) {
        ...
        return List.of(new SynchedEntityData.DataValue<>(accessor.getId(), EntityDataSerializers.COMPONENT, text));
    }
     ...
     * A probe value is written through the save-data path, then the single entry using the
    ...
    private EntityDataAccessor<Component> discoverTextAccessor() {
        ...
        // and so "alignment" is always present (vanilla logs an error when it is missing).
