...
public class HologramsInsertLineCommand implements Command<CommandSourceStack> {
    ...
    public int executeCommand(CommandContext<CommandSourceStack> context, String[] args) throws CommandSyntaxException {
        if (args.length < 3) {
            context.getSource().sendSystemMessage(Component.literal("§c§l(!) §cUsage: /eh insertline <id> <line> <text>"));
            return 0;
        }
        ...
        try {
            ...
            player.sendSystemMessage(Component.literal("§a§l(!) §aInserted line at position §f" + lineNumber + "§a in hologram '§f" + id + "§a'!"));
            return Command.SINGLE_SUCCESS;
        } catch (Exception e) {
            player.sendSystemMessage(Component.literal("§c§l(!) §cError inserting line: §f" + e.getMessage()));
            return 0;
        }
    }
} 