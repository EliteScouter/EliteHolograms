...
public class HologramsInfoCommand implements HologramsCommand.SubCommand {
    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (hologram instanceof com.strictgaming.elite.holograms.neo262.hologram.implementation.NeoForgeHologram nf) {
            source.sendSuccess(() -> UtilChatColour.parse("&3│ &bDisplay: &f"
                    + nf.getDisplayType().getSerializedName()), false);

            if (nf.getDisplayType()
                    == com.strictgaming.elite.holograms.neo262.hologram.HologramDisplayType.FIXED) {
